...
class Plotter:
    def __init__(self, track, heatmap='', hide_angles=False, show_all_angles=False):
        """
        :param track: [string] Image filename(png) of the track to overlay.
    ...
    def _generate_targets(self, idx, angle_type, num_points):
    ...
    def get_target_points(self):
        for i, _ in enumerate(waypoints):
            self.target_points += self._generate_targets(i, *targets_refs[i])
    ...
    def key_event(self, e, ax, fig):
        ...
        ax.cla()
        plt.imshow(plt.imread(self.track), extent=[0, 8, 0, 5.2])
    ...
    def plot(self):
        ...
        self._draw_lines(0)
        if not self.heatmap:
            fig.canvas.mpl_connect('key_press_event', lambda event: self.key_event(event, ax, fig))
            ax = fig.add_subplot(111)
        plt.show()


if __name__ == '__main__':
    Plotter('reinvent2018.png', hide_angles=True)
    Plotter('reinvent2018.png', show_all_angles=True)
    Plotter('reinvent2018.png')
    Plotter('reinvent2018.png', heatmap='Reward')
    Plotter('reinvent2018.png', heatmap='Speed')
